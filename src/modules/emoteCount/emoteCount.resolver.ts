...
export class EmoteCountResolver {
  ...
  async emoteCount(@Arg("userId") userId: string, @Arg("emoteId") emoteId: string ): Promise<EmoteCount | null> {
    return await this.emoteCountService.getByUserIdEmoteId(userId, emoteId);
  }

  @Mutation(returns => EmoteCount, { nullable: true })
  async saveEmoteCount(@Arg("userId") userId: string, @Arg("emoteId") emoteId: string, @Arg("add", type => Int) add: number) {
    return await this.emoteCountService.saveOrUpdateEmoteCount(userId, emoteId, add);
  }
}